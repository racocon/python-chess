#initialize the global variables to keep up with the states in chess
...
class Board:
    ...
    #initialize the 8x8 chessboard and the pieces position
    #lowercase letters -> black pieces; uppercase letters -> white pieces
    chessBoard =   [["r", "n", "b", "q", "k", "b", "n", "r"],
                    ["p", "p", "p", "p", "p", "p", "p", "p"], #r -> black rook;   R -> white rook
                    [" ", " ", " ", " ", " ", " ", " ", " "], #n -> black knight; N -> white knight
                    [" ", " ", " ", " ", " ", " ", " ", " "], #b -> black bishop; B -> white bishop
                    [" ", " ", " ", " ", " ", " ", " ", " "], #q -> black queen;  Q -> white queen
                    [" ", " ", " ", " ", " ", " ", " ", " "], #k -> black king;   K -> white king
                    ["P", "P", "P", "P", "P", "P", "P", "P"], #p -> black pawn;   P -> white pawn
                    ["R", "N", "B", "Q", "K", "B", "N", "R"]]
    ...
    def determinePiece(self, move2compare):
        ...
        #this following statement checks for king's movement over 1 tile
        if(self.pieceOnBoard == "K" or self.pieceOnBoard == 'k') and (self.y2 - self.y1 == 2 or self.y2 - self.y1 == -2):

            if KingcastlingStateBlack == True or KingcastlingStateWhite == True or QueencastlingStateBlack == True or QueencastlingStateWhite == True:
                ...
                if self.pieceOnBoard =="K" and (QueencastlingStateWhite == True or KingcastlingStateWhite == True):
                    ...
                    #if the above check are passed, the castling function will be called by passing the necessary parameters
    ...
    #this function determines whether the move made is possible or not by comparing with all the possible moves on the board for the particular piece
    ...
    #the function takes the move made by the user and a number representing whether it's a castling move
    #castling move made for the white piece is denoted by the integer "1", for the black piece "2", and for not castling move "3"
    def makeMove(self, proposedMove, castlingState):
        ...
        #this statement will initialize the following variable as "White" if the move was made bu the black piece player and "Black" if otherwise
        ...
        #for white piece
        #if the piece moved was a king or a rook, set the castling state to false. If a rook is moved, the rook will be checked first and then set the state accordingly
        if self.getChar == "K" or self.getChar == "R":
            #if it is the rook that has been moved, the rook will be identified first
            if self.getChar == "R":
        ...
        #check if te move is made by the white piece player
        ...
        if (ord(self.getChar) >= 65 and ord(self.getChar) <= 90) and whiteMoveMade == False:
            ...
            tempPiece = self.chessBoard[self.x2][self.y2]       #keep the piece of the tile the move was made in the variable
            self.chessBoard[self.x2][self.y2] = self.getChar    # make the tile where the move was made occupied with the piece that was selected to make a move
            whiteMoveMade = True                                #set the variable to indicate that the white piece player has made a move

            if self.CheckState("White") == True: #if the white piece player's move exposes the King, them the move will be retreated back
                ...
                #check if the pawn has made it to promotion
                self.Promotion_Check("White")

        #check if the move is made by the black piece player
        #the variable whiteMoveMade = True indicates that black piece player has yet to make a move
            ...
            if self.CheckState("Black") == True: #if the black piece player's move exposes the King, them the move will be retreated back
                self.chessBoard[self.x1][self.y1] = self.getChar
                self.chessBoard[self.x2][self.y2] = tempPiece
                whiteMoveMade = True #returns the value back to true
            else:
                #check if the pawn has made it to promotion
        ...
        #CheckState() is a function to check if the move made has "Checked" the opponent's King
        ...
        #if the CheckState() function returns true, Check_CheckMate() is called to check if a checkMate is made
    ...
    def possibleMoves(self, piece):
        ...
        for i in range(64):
            ...
            if self.chessBoard[int(i/8)][i%8] == "N" and self.piece == "N":
            ...
            if self.chessBoard[int(i/8)][i%8] == "B" and self.piece == "B":
            ...
            if self.chessBoard[int(i/8)][i%8] == "Q" and self.piece == "Q":
            ...
            if self.chessBoard[int(i/8)][i%8] == "K" and self.piece == "K":
                movesList += self.KingMove(i, "White")

            #check if there's a black king at the coordinate
            if self.chessBoard[int(i/8)][i%8] == "k" and self.piece == "k":
                movesList += self.KingMove(i, "Black")

            #check if there's a white pawn at the coordinate
            if self.chessBoard[int(i/8)][i%8] == "P" and self.piece == "P":
    ...
    ################### BEGINNING OF WHITE PAWN'S MOVEMENTS ###################
    ...
    def WhitePawnMove(self, position):
        ...
        #get the coordinates
        r = int(position / 8)   #row or x-axis
        c = position % 8        #column or y-axis

        #row 6 is white pawn's initial position
        if r == 6:
            #looping for 2 boxes ahead
            for x in range(5, 3, -1):
                #Try-Exception is used to avoid errors regarding out of bounds index when the coordinates are beyond the chessboard
                try:
                    #check whether the box in front is empty
        ...
        if r-1 >= 0:
            #Try-Exception is used to avoid errors regarding out of bounds index when the coordinates are beyond the chessboard
            try:
                #test whether the pawns can take any materials (ord function crosschecks with ASCII chars for a-z [lowercase])
    ...
    ################### END OF WHITE PAWN'S MOVEMENTS ###################




    ################### BEGINNING OF BLACK PAWN'S MOVEMENTS ###################

    #function to calculate the legal moves for black pawns
    def BlackPawnMove(self, position):
        ...
        if r+1 >= 0 and c-1 >= 0:
            #Try-Exception is used to avoid errors regarding out of bounds index when the coordinates are beyond the chessboard
            try:
                #test whether the pawns can take any materials (ord function crosschecks with ASCII chars for A-Z [uppercase])
    ...
    ################### END OF BLACK PAWN'S MOVEMENTS ###################




    ################### BEGINNING OF ROOK'S MOVEMENTS ###################

    #function to calculate the legal moves for rooks
    def RookMove(self, position, colour):

        #set the variables according to colour of the piece
        if colour == "White":
            #these values represent the range of lowercase letters (a-z)
            ordA, ordB = 97, 122
        elif colour == "Black":
            #these values represent the range of uppercase letters (A-Z)
            ordA, ordB = 65, 90
        ...
        #this loops iterates from -1 to 1 to change the direction of the iterations (bottom, top, right, left)
        for j in range(-1,2,2):
            ...
            try:
                ...
                #check whether there is an enemy's piece to be captured
    ...
    ################### END OF ROOK'S MOVEMENTS ###################




    ################### BEGINNING OF KNIGHT'S MOVEMENTS ###################

    #function to calculate the legal moves for knights
    def KnightMove(self, position, colour):
        ...
        #this nested loops will complete all the 8 possible route for a knight
        for j in range(-1,2,2):
    ...
    ################### END OF KINGHT'S MOVEMENTS ###################




    ################### BEGINNING OF BISHOP'S MOVEMENTS ###################

    #function to calculate the legal moves for bishops
        ...
        for j in range(-1,2,2):
            ...
            for k in range(-1,2,2):
                ...
                try:
                    ...
                    #check whether there is an enemy's piece to be captured
                    if ord(self.chessBoard[r+temp*j][c+temp*k]) >= ordA and ord(self.chessBoard[r+temp*j][c+temp*k]) <= ordB:
                        #if statement to make sure the value of the row does not go below 0
                        if r+temp*j >= 0 and c+temp*k >= 0:
                            #append the legal moves to the list
    ...
    ################### END OF BISHOP'S MOVEMENTS ###################




    ################### BEGINNING OF QUEEN'S MOVEMENTS ###################

    #function to calculate the legal moves for queen
    def QueenMove(self, position, colour):
        ...
        #nested loop that implements the movement of a rook + bishop
    ...
    ################### END OF QUEEN'S MOVEMENTS ###################




    ################### BEGINNING OF KING'S MOVEMENTS ###################

    #function to calculate the legal moves for king
    ...
    def Castling(self, colour, x1, y1, x2, y2):
        ...
        #check to see if the castling move was made by the white piece player
        #x1 = 7 and y1 = 4 indicates that King is in the initial position
        if colour == "White" and x1 == 7 and y1 == 4:
            ...
            if x2 == 7 and y2 == 6:
                ...
                if self.chessBoard[7][7] == "R" and self.chessBoard[7][5] == " " and self.chessBoard[7][6] == " ":
                    ...
                    for evety_set in self.Check_Opponent_Threat("White", "Threat"):
                    ...
                    if castlingAbility == True:
                        ...
                        #calling the makeMove function the 2nd time with parameter 3 to denote the end of castling move
            ...
            #x2 = 7 and y2 = 2 indicates that the King is moved to the left side 2 tiles (Queen's side castling)
            elif x2 == 7 and y2 == 2:
                #check if there exists a rook at the right end and the tiles between are empty
        ...
        #check to see if the castling move was made by the black piece player
        #x1 = 0 and y1 = 4 indicates that King is in the initial position
        if colour == "Black" and x1 == 0 and y1 == 4:
            ...
            if x2 == 0 and y2 == 6:
                ...
                if self.chessBoard[0][7] == "r" and self.chessBoard[0][6] == " " and self.chessBoard[0][5] == " ":
                    ...
                    setB = [0,5]
                    #Check_Opponent_Threat() is a function that detects all the threats that are made by a certain player
                    for evety_set in self.Check_Opponent_Threat("Black", "Threat"):
                        #if there exists a threat on the tiles that are about to be passed by the King, the castlingAbility variable will be set to false to deny the castling move
                        if setA == every_set:
                            castlingAbility = False
                        if setB == every_set:
    ...
    #this function can also be used to return all the possible moves that can be made by a player, thus the parameter desired is used accordingly
    #note that this function is not the same as the possibleMoves() function as possibleMoves() function can only be passed with one single character at one time (Means can only calculate the possible moves of one particular piece)
    def Check_Opponent_Threat(self, colour, desire):
        ...
        #elif the parameter was passed in with "White" colour, then the possible moves for white will be generated and vice versa
        elif colour == "Black":
            #iterate through all the 6 pieces
            for i in range(6):
        ...
        if desire == "PossibleMovesOnly":
            ...
            for k in range(len(all_the_possible_moves)):
                #variable initialization is made here so that it will be emptied every time a loop of k is made
                temp = []
                temp1 = []
    ...
    #this function is used to check whether a player is in Check
    def CheckState(self, colour):
        ...
        #check if there exists a "K" or a "k" in the threats
        for each in threat:
            #comparison
    ...
    #this function is to check whether a player has been checkmated
    #this is the function that fires when a player is in check
    def Check_CheckMate(self, colour):

        #checkmate test for white
        if colour == "White":
            #by giving the parameter "PossibleMovesOnly" to Check_Opponent_Threat() function, the function will return only the possible moves, not the threats
